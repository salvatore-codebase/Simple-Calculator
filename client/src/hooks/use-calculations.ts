...
export function useCreateCalculation() {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: [api.calculations.list.path] }),
  });
}
...
export function useClearCalculations() {
  ...
  return useMutation({
    mutationFn: async () => {
      const res = await fetch(api.calculations.clear.path, {
        method: api.calculations.clear.method,
        credentials: "include",
      });
